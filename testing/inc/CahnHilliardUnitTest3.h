...
#ifndef CAHN_HILLIARD_UNIT_TEST_3_H_
#define CAHN_HILLIARD_UNIT_TEST_3_H_ 
 ...
 */
template <int dim>
class PhaseInitialCondition3 : public Function<dim>{
public:
    PhaseInitialCondition3();
    
    virtual double value(const Point<dim> &p, const unsigned int) const;
};

/**
 * Class which encapsulates the exact phase solution.
 */
template <int dim>
class PhaseExactSolution3 : public Function<dim>{
public:
    PhaseExactSolution3() : Function<dim>(){}
 ...
 */
template <int dim>
class ChemicalPotentialForcingFunction3 : public Function<dim>{
public:
    ChemicalPotentialForcingFunction3() : Function<dim>(){}
 ...
 */
class MatrixInverseAction3{
public:
    MatrixInverseAction3();
 ...
 */
class CHReducedPhaseMatrixAction3{
public:
    CHReducedPhaseMatrixAction3();
...
private:
    ...
    MatrixInverseAction3 MassInverse;
...
/**
 * Class which encapsulates the third unit test for Cahn-Hilliard. It computes the error and convergence rate
 * of the phase for a particular forced solution on a sequence of globally refined meshes.
 */
template <int dim>
class CahnHilliardUnitTest3{
public:
    CahnHilliardUnitTest3();
...
private:
    ...
    /* Number of DoFs for the Phase and Chemical Potential. */
    unsigned int CHNumOfDoFs;
    /* Degree of the finite element used. */
    unsigned int CHDegree;
    ...
    /* Sparsity pattern */
    SparsityPattern preconditionerSparsityPattern;
    /* Index set for all all indices */
    IndexSet CHAllIndex;
    
    //Variables related to the matrices and RHSs of the discretization
    PhaseInitialCondition3<dim> phaseInitialCondition;
    /* Mass matrix */
    ...
    TrilinosWrappers::SparseMatrix CHPhaseConstantPreconditioner;
    /* Complete preconditioner for the phase equation. */
    TrilinosWrappers::SparseMatrix CHPhaseTimeDepPreconditioner;
    /* Object to compute the action of the reduced phase matrix. */
    CHReducedPhaseMatrixAction3 CHReducedPhaseMatrix;
    /* Object to compute the action of the inverse of the mass matrix. */
    MatrixInverseAction3 CHMassInverse;
    ...
    //Variables related to the Phase and Chemical Potential solutions
