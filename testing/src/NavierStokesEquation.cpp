...
void NavierStokesEquation<dim>::run(){
    ...
    std::string error_filename = "NSerror.tex";
    ...
    std::cout << std::endl;
    convergence_table.write_text(std::cout);
    std::string conv_filename = "NSconvergence.tex";
    std::ofstream table_file(conv_filename.c_str());
    convergence_table.write_tex(table_file);
}            

/**
 * Computes the H^1-seminorm and L^2 errors of the velocity at the current global refinement level.
 */
template<int dim>
void NavierStokesEquation<dim>::processSolution(const unsigned int cycle){
    ...
    convergence_table.add_value("L2", velocityL2Error);
    convergence_table.add_value("H1", velocityH1Error);
